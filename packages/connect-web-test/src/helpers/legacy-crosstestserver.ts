...
export const legacyCrosstestTransports: Record<
  string,
  (options?: Record<string, unknown>) => Transport
> = {
  "legacy gRPC-web transport": (options) =>
    createGrpcWebTransport({
      ...options,
      baseUrl,
    }),
  "legacy connect JSON transport": (options) =>
    createConnectTransport({
      ...options,
      baseUrl,
      useBinaryFormat: false,
    }),
  "legacy connect binary transport": (options) =>
    createConnectTransport({
      ...options,
      baseUrl,
